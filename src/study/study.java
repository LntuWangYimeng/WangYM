...
public class study {
	public static void main(String args[]) throws IOException {
		FileInputStream fis = new FileInputStream("C:\\Users\\86176\\Desktop\\1.txt");
		int b;
		while ((b = fis.read()) != -1) {
	...
	public static void main1(String[] args) {
		File srcFile = new File("C:\\Users\\86176\\Desktop\\1.txt");
		String key = "aaaaaaaa";
		des des = new des();
		String ciphertext = des.encrypt(srcFile, key, "encrypt");
		System.out.println("明文为：" + srcFile + "\n密钥为：" + key + "\n加密为：" + ciphertext + "\n");
		System.out.println("解密为：" + des.encrypt(srcFile, key, "decrypt"));
	}
	...
	// 2 加密
	public String encrypt(File srcFile, String key, String type) {
        StringBuffer ciphertext = new StringBuffer(); // 存储密文

        // 2.1 把明文转换成二进制字符串
        StringBuffer plaintextBinary = new StringBuffer(); // 存储明文二进制
        for (int i = 0; i < 8; ++i) {
            StringBuffer mSubPlaintextTemp = new StringBuffer(Integer.toBinaryString(((CharSequence) srcFile).charAt(i)));
